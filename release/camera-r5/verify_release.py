...
APP_SOURCE_COMMIT = "40493849af148124a45b6a6c65d9c9146c03fdb4"
ROLLBACK_SOURCE_COMMIT = "b6a3c2519c78d83669444c31c7c139920a1c4506"
RELEASE_TAG = "camera-experimental-r5"
PUBLIC_URL = "https://aldaqrouqtaysir.github.io/plategauge/"
APPROVER = "Taysir Al Daqrouq"
APPROVAL_PATH = "release/camera-r5/approval.json"
INVENTORY_PATH = "release/camera-r5/inventory.json"
ROLLBACK_INVENTORY_PATH = "release/camera-r5/rollback-inventory.json"
EVIDENCE_PATH = "release/camera-r5/verification-evidence.json"
...
OPERATIONS_FILES = frozenset(
    {
        ...
        "docs/CAMERA_R5_RELEASE.md",
        ".github/workflows/release-camera-r5-pages.yml",
        ".github/workflows/weekly-camera-r5-smoke.yml",
        ".github/workflows/rollback-camera-r5-pages.yml",
...
def verify_approval(
    ...
    if any(
        path not in OPERATIONS_FILES and not path.startswith("release/camera-r5/") for path in changed
    ):
    ...
    for row in (
        ...
        for path in _changed_paths(root, commits[1], commits[0]):
            if path not in OPERATIONS_FILES and not path.startswith("release/camera-r5/"):
    ...
    for path, (previous_guard, next_guard) in WORKFLOW_GUARDS.items():
        if path in changed:
            ...
            if original.count(previous_guard) != 1 or current != original.replace(
                ...
                raise VerificationError(
                    "Historical workflow may only replace its r1+r2+r3+r4 guard with the exact r1+r2+r3+r4+r5 guard"
                )
