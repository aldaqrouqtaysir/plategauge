...
class ApprovalTests(ReleaseFixture):
    ...
    def test_removing_historical_exclusions_instead_of_adding_r4_is_blocked(self) -> None:
        path = ".github/workflows/weekly-smoke.yml"
        previous_guard, _ = gate.WORKFLOW_GUARDS[path]
        file = self.repo / path
        file.write_bytes(file.read_bytes().replace(
            previous_guard,
            b"    if: ${{ vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r4' }}\n",
        ))
        self.commit("forbidden loss of r1, r2 and r3 routing")
        self.operations = self.git("rev-parse", "HEAD").strip()
        self.approve()
        with self.assertRaisesRegex(gate.VerificationError, "Historical workflow"):
            self.verify()
    ...
    def test_r1_release_records_cannot_change_in_r4_operations(self) -> None:
    ...
    def test_r2_release_records_cannot_change_in_r4_operations(self) -> None:
    ...
    def test_r3_release_records_cannot_change_in_r4_operations(self) -> None:
