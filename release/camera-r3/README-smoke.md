...
This release-operations harness does not change the app. It checks the exact archived camera build from source `62efadcfd342a193458036912b38b0a613bf94be`; never rebuild that artifact for this check. Running the HTTPS mode requires separate release authorization. The presence of this harness grants none.
...
```text
pnpm exec tsc -p ../release/camera-r3/tsconfig.json
pnpm exec playwright test --config ../release/camera-r3/playwright.live.config.ts
```
