...
APP_SOURCE_COMMIT = "62efadcfd342a193458036912b38b0a613bf94be"
ROLLBACK_SOURCE_COMMIT = "b6a3c2519c78d83669444c31c7c139920a1c4506"
RELEASE_TAG = "camera-experimental-r3"
PUBLIC_URL = "https://aldaqrouqtaysir.github.io/plategauge/"
APPROVER = "Taysir Al Daqrouq"
APPROVAL_PATH = "release/camera-r3/approval.json"
INVENTORY_PATH = "release/camera-r3/inventory.json"
ROLLBACK_INVENTORY_PATH = "release/camera-r3/rollback-inventory.json"
EVIDENCE_PATH = "release/camera-r3/verification-evidence.json"
...
OPERATIONS_FILES = frozenset(
    {
        ...
        "docs/CAMERA_R3_RELEASE.md",
        ".github/workflows/release-camera-r3-pages.yml",
        ".github/workflows/weekly-camera-r3-smoke.yml",
        ".github/workflows/rollback-camera-r3-pages.yml",
...
def verify_approval(
    ...
    if any(
        path not in OPERATIONS_FILES and not path.startswith("release/camera-r3/") for path in changed
    ):
    ...
    for row in (
        ...
        for path in _changed_paths(root, commits[1], commits[0]):
            if path not in OPERATIONS_FILES and not path.startswith("release/camera-r3/"):
    ...
    for path, (previous_guard, next_guard) in WORKFLOW_GUARDS.items():
        if path in changed:
            ...
            if original.count(previous_guard) != 1 or current != original.replace(
                ...
                raise VerificationError(
                    "Historical workflow may only replace its r1+r2 guard with the exact r1+r2+r3 guard"
                )
