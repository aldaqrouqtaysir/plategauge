...
class ReleaseFixture(unittest.TestCase):
    def setUp(self) -> None:
        ...
        for name in (
            gate.REQUIRED_FILES | gate.CAMERA_FILES | {"assets/index-a.js", "assets/index-a.css"}
        ):
...
class InventoryTests(ReleaseFixture):
    ...
    def test_known_rollback_profile_has_no_camera_notice_requirement(self) -> None:
        ...
        payload["files"] = [
            entry for entry in payload["files"] if entry["path"] not in gate.CAMERA_FILES
        ]
...
class ApprovalTests(ReleaseFixture):
    def setUp(self) -> None:
        ...
        payload["appSourceCommit"] = gate.ROLLBACK_SOURCE_COMMIT
        write_json(self.repo / gate.ROLLBACK_INVENTORY_PATH, payload)
    ...
    def test_removing_r1_exclusion_instead_of_adding_r2_is_blocked(self) -> None:
        path = ".github/workflows/weekly-smoke.yml"
        previous_guard, _ = gate.WORKFLOW_GUARDS[path]
        file = self.repo / path
        file.write_bytes(file.read_bytes().replace(
            previous_guard,
            b"    if: ${{ vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r2' }}\n",
        ))
        self.commit("forbidden loss of r1 routing")
        self.operations = self.git("rev-parse", "HEAD").strip()
        self.approve()
        with self.assertRaisesRegex(gate.VerificationError, "Historical workflow"):
            self.verify()
    ...
    def test_r1_release_records_cannot_change_in_r2_operations(self) -> None:
