...
APP_SOURCE_COMMIT = "5d67e29bf79d088e7c4a6903c6ce2c5c58a10f1a"
ROLLBACK_SOURCE_COMMIT = "b6a3c2519c78d83669444c31c7c139920a1c4506"
RELEASE_TAG = "camera-experimental-r2"
PUBLIC_URL = "https://aldaqrouqtaysir.github.io/plategauge/"
APPROVER = "Taysir Al Daqrouq"
APPROVAL_PATH = "release/camera-r2/approval.json"
INVENTORY_PATH = "release/camera-r2/inventory.json"
ROLLBACK_INVENTORY_PATH = "release/camera-r2/rollback-inventory.json"
EVIDENCE_PATH = "release/camera-r2/verification-evidence.json"
...
OPERATIONS_FILES = frozenset(
    {
        ...
        "docs/CAMERA_R2_RELEASE.md",
        ".github/workflows/release-camera-r2-pages.yml",
        ".github/workflows/weekly-camera-r2-smoke.yml",
        ".github/workflows/rollback-camera-r2-pages.yml",
...
WORKFLOW_GUARDS = {
    ".github/workflows/weekly-smoke.yml": (
        b"    if: ${{ vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r1' }}\n",
        b"    if: ${{ vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r1' && vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r2' }}\n",
    ),
    ".github/workflows/release-pages.yml": (
        b"    if: ${{ github.event_name != 'release' || github.event.release.tag_name != 'camera-experimental-r1' }}\n",
...
def validate_inventory(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    required = REQUIRED_FILES | (
        CAMERA_FILES if payload["appSourceCommit"] == APP_SOURCE_COMMIT else frozenset()
    )
    if (
...
def verify_approval(
    ...
    if any(
        path not in OPERATIONS_FILES and not path.startswith("release/camera-r2/") for path in changed
    ):
    ...
    for row in (
        ...
        for path in _changed_paths(root, commits[1], commits[0]):
            if path not in OPERATIONS_FILES and not path.startswith("release/camera-r2/"):
    ...
    for path, (previous_guard, next_guard) in WORKFLOW_GUARDS.items():
        if path in changed:
            ...
            if original.count(previous_guard) != 1 or current != original.replace(
                ...
                raise VerificationError(
                    "Historical workflow may only replace its r1 guard with the exact r1+r2 guard"
                )
