...
This release-operations harness does not change the app. It checks the exact archived camera build from source `5d67e29bf79d088e7c4a6903c6ce2c5c58a10f1a`; never rebuild that artifact for this check. Running the HTTPS mode requires separate release authorization. The presence of this harness grants none.
...
```text
pnpm exec tsc -p ../release/camera-r2/tsconfig.json
pnpm exec playwright test --config ../release/camera-r2/playwright.live.config.ts
```
...
The inventory has strict shape `{schemaVersion:1, appSourceCommit:"5d67e29bf79d088e7c4a6903c6ce2c5c58a10f1a", files:[{path:"relative/POSIX",size:123,sha256:"lowercase64hex"}]}`. It contains exactly 46 sorted, unique, bounded paths. Its exact raw-byte SHA-256 is pinned to `d885da02a49fdc54863656ff3c368b41aa8c932c2446bd3bfca6826e9de3009b`. Structural validation also pins the unchanged model and frozen benchmark-evidence identities. The inventory digest must come from release approval, not a fresh download of a mutable live manifest.
...
The evidence-route check also requires the requested two-paragraph footer block
to be absent while the accessible Privacy and AI-assistance disclosure links
remain intact. The presentation change does not remove the linked records or
alter the frozen model, data, scientific results or historical release artifacts.
