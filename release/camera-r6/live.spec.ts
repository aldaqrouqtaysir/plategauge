...
test("capture skip link is painted above navigation and reaches main by keyboard", async ({ page, context }) => {
  ...
  await expect(page.getByText(/Take two photos\. Match the framing\./i)).toHaveCount(0);
  const boundary = page.locator("#rc-capture-disclosure");
  await expect(boundary).toBeVisible();
  await expect(boundary).toContainText("Experimental estimate.");
  await expect(boundary).toContainText(/Not validated for your photos or field use; not a scale measurement/);
  const privacy = page.locator("details.rc-privacy-details");
  await expect(privacy).toHaveCount(1);
  await expect(privacy.locator("summary")).toHaveText("Privacy & storage");
...
test("modified Evidence click preserves generated capture until current-document exit", async ({ page, context }) => {
  ...
  await page.getByRole("link", { name: "Evidence", exact: true }).click({ modifiers: ["ControlOrMeta"] });
  const popup = await popupPromise;
  await popup.waitForLoadState("domcontentloaded");
  await expect(popup).toHaveURL(smokeConfiguration().baseURL + "?view=evidence");
  await expect(popup.getByRole("complementary", { name: "About these results", exact: true })).toBeVisible();
...
test("explicit generated session survives reload without automatic camera or inference", async ({ page, context }) => {
  ...
  await expect(page.getByTestId("session-controls")).toContainText(/clearing this page does not delete the file/i);
  await expectPrivate(page, network);
});
