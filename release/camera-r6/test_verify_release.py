...
class ApprovalTests(ReleaseFixture):
    ...
    def test_removing_historical_exclusions_instead_of_adding_r6_is_blocked(self) -> None:
        path = ".github/workflows/weekly-smoke.yml"
        previous_guard, _ = gate.WORKFLOW_GUARDS[path]
        file = self.repo / path
        file.write_bytes(file.read_bytes().replace(
            previous_guard,
            b"    if: ${{ vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r6' }}\n",
        ))
        self.commit("forbidden loss of r1, r2, r3, r4 and r5 routing")
        self.operations = self.git("rev-parse", "HEAD").strip()
        self.approve()
        with self.assertRaisesRegex(gate.VerificationError, "Historical workflow"):
            self.verify()
    ...
    def test_r1_release_records_cannot_change_in_r6_operations(self) -> None:
    ...
    def test_r2_release_records_cannot_change_in_r6_operations(self) -> None:
    ...
    def test_r3_release_records_cannot_change_in_r6_operations(self) -> None:
        old_record = self.repo / "release/camera-r3/approval.json"
        old_record.parent.mkdir(parents=True, exist_ok=True)
        old_record.write_bytes(b"synthetic unauthorized historical rewrite\n")
        self.commit("forbidden r3 historical approval path change")
        self.operations = self.git("rev-parse", "HEAD").strip()
        self.approve()
        with self.assertRaisesRegex(gate.VerificationError, "protected"):
            self.verify()

    def test_r4_release_records_cannot_change_in_r6_operations(self) -> None:
        old_record = self.repo / "release/camera-r4/approval.json"
        old_record.parent.mkdir(parents=True, exist_ok=True)
        old_record.write_bytes(b"synthetic unauthorized historical rewrite\n")
        self.commit("forbidden r4 historical approval path change")
        self.operations = self.git("rev-parse", "HEAD").strip()
        self.approve()
        with self.assertRaisesRegex(gate.VerificationError, "protected"):
            self.verify()

    def test_r5_release_records_cannot_change_in_r6_operations(self) -> None:
        old_record = self.repo / "release/camera-r5/approval.json"
        old_record.parent.mkdir(parents=True, exist_ok=True)
        old_record.write_bytes(b"synthetic unauthorized historical rewrite\n")
        self.commit("forbidden r5 historical approval path change")
        self.operations = self.git("rev-parse", "HEAD").strip()
        self.approve()
        with self.assertRaisesRegex(gate.VerificationError, "protected"):
            self.verify()
