...
APP_SOURCE_COMMIT = "d2fc03257e8ebfdae3923aa454fa8b3645212731"
ROLLBACK_SOURCE_COMMIT = "b6a3c2519c78d83669444c31c7c139920a1c4506"
RELEASE_TAG = "camera-experimental-r6"
PUBLIC_URL = "https://aldaqrouqtaysir.github.io/plategauge/"
APPROVER = "Taysir Al Daqrouq"
APPROVAL_PATH = "release/camera-r6/approval.json"
INVENTORY_PATH = "release/camera-r6/inventory.json"
ROLLBACK_INVENTORY_PATH = "release/camera-r6/rollback-inventory.json"
EVIDENCE_PATH = "release/camera-r6/verification-evidence.json"
...
OPERATIONS_FILES = frozenset(
    {
        ...
        "docs/CAMERA_R6_RELEASE.md",
        ".github/workflows/release-camera-r6-pages.yml",
        ".github/workflows/weekly-camera-r6-smoke.yml",
        ".github/workflows/rollback-camera-r6-pages.yml",
...
WORKFLOW_GUARDS = {
    ".github/workflows/weekly-smoke.yml": (
        b"    if: ${{ vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r1' && vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r2' && vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r3' && vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r4' && vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r5' }}\n",
        b"    if: ${{ vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r1' && vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r2' && vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r3' && vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r4' && vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r5' && vars.PLATEGAUGE_ACTIVE_PROFILE != 'camera-experimental-r6' }}\n",
    ),
    ".github/workflows/release-pages.yml": (
        b"    if: ${{ github.event_name != 'release' || (github.event.release.tag_name != 'camera-experimental-r1' && github.event.release.tag_name != 'camera-experimental-r2' && github.event.release.tag_name != 'camera-experimental-r3' && github.event.release.tag_name != 'camera-experimental-r4' && github.event.release.tag_name != 'camera-experimental-r5') }}\n",
        b"    if: ${{ github.event_name != 'release' || (github.event.release.tag_name != 'camera-experimental-r1' && github.event.release.tag_name != 'camera-experimental-r2' && github.event.release.tag_name != 'camera-experimental-r3' && github.event.release.tag_name != 'camera-experimental-r4' && github.event.release.tag_name != 'camera-experimental-r5' && github.event.release.tag_name != 'camera-experimental-r6') }}\n",
    ),
}
...
def verify_approval(
    ...
    if any(
        path not in OPERATIONS_FILES and not path.startswith("release/camera-r6/") for path in changed
    ):
    ...
    for row in (
        ...
        for path in _changed_paths(root, commits[1], commits[0]):
            if path not in OPERATIONS_FILES and not path.startswith("release/camera-r6/"):
    ...
    for path, (previous_guard, next_guard) in WORKFLOW_GUARDS.items():
        if path in changed:
            ...
            if original.count(previous_guard) != 1 or current != original.replace(
                ...
                raise VerificationError(
                    "Historical workflow may only replace its r1+r2+r3+r4+r5 guard with the exact r1+r2+r3+r4+r5+r6 guard"
                )
