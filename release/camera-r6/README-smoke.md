...
This release-operations harness does not change the app. It checks the exact archived camera build from source `d2fc03257e8ebfdae3923aa454fa8b3645212731`; never rebuild that artifact for this check. Running the HTTPS mode requires separate release authorization. The presence of this harness grants none.
...
```text
pnpm exec tsc -p ../release/camera-r6/tsconfig.json
pnpm exec playwright test --config ../release/camera-r6/playwright.live.config.ts
```
...
The inventory has strict shape `{schemaVersion:1, appSourceCommit:"d2fc03257e8ebfdae3923aa454fa8b3645212731", files:[{path:"relative/POSIX",size:123,sha256:"lowercase64hex"}]}`. It contains exactly 45 sorted, unique, bounded paths. Its exact raw-byte SHA-256 is pinned to `a95a1770aaae2373887fd3bc38d8adcd8f9f856c6b02c41bd92ebec3f68c2fc8`. Structural validation also pins the unchanged model and frozen benchmark-evidence identities. The inventory digest must come from release approval, not a fresh download of a mutable live manifest.

## What is checked

Eight checks comprise non-browser preflight and retired-resource tests, plus six bounded Chromium flows: inert startup; visibly painted keyboard skip navigation at a narrow viewport; a modified Evidence-link click preserving a generated capture until actual exit; generated before/after captures followed by explicit real pinned-model inference and optional mass arithmetic; explicit generated-session download/restore/clear; and navigation to evidence plus inert unknown/ambiguous routes. The product is not given a fake model. The test installs a synthetic camera before application code, then independently checks and locks that generated function before any camera action. It never calls native `getUserMedia`, requests permission, or uses personal images. A failed sentinel stops the test. The HTML Evidence popup is restricted by the same exact-inventory network guard; this adds no destination or file permission.

The capture-path check also requires the removed floating hero sentence to be
absent, the experimental/unvalidated boundary to remain visible, and the
Privacy & storage summary to be visible with its details closed by default.
These presentation checks neither activate a camera nor request a model.

The modified-click check targets the HTML Evidence route, not a raw Markdown
notice whose viewing/download behavior can depend on the browser and host. It
checks the actual new-tab interaction, capture retention in the original tab,
and cleanup on normal departure. Footer modifier behavior also has unit tests;
this smoke does not assert that every browser opens raw notices as page tabs.
