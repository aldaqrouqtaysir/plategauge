...
APP_SOURCE_COMMIT = "ca88b440636b8028e60a1e79ae6a51b0539a37ca"
ROLLBACK_SOURCE_COMMIT = "b6a3c2519c78d83669444c31c7c139920a1c4506"
RELEASE_TAG = "camera-experimental-r1"
PUBLIC_URL = "https://aldaqrouqtaysir.github.io/plategauge/"
APPROVER = "Taysir Al Daqrouq"
APPROVAL_PATH = "release/camera/approval.json"
INVENTORY_PATH = "release/camera/inventory.json"
ROLLBACK_INVENTORY_PATH = "release/camera/rollback-inventory.json"
EVIDENCE_PATH = "release/camera/verification-evidence.json"
...
OPERATIONS_FILES = frozenset(
    {
        "README.md",
        "docs/CAMERA_RELEASE.md",
        "docs/CAMERA_RELEASE_APPROVAL.md",
        ".github/workflows/release-camera-pages.yml",
        ".github/workflows/weekly-camera-smoke.yml",
        ".github/workflows/rollback-camera-pages.yml",
...
def verify_approval(
    ...
    if any(
        path not in OPERATIONS_FILES and not path.startswith("release/camera/") for path in changed
    ):
    ...
    for row in (
        ...
        for path in _changed_paths(root, commits[1], commits[0]):
            if path not in OPERATIONS_FILES and not path.startswith("release/camera/"):
    ...
    for path, (anchor, guard) in WORKFLOW_GUARDS.items():
        if path in changed:
            original = _git(root, "show", f"{APP_SOURCE_COMMIT}:{path}")
            current = _git(root, "show", f"{operations}:{path}")
            if original.count(anchor) != 1 or current != original.replace(
                anchor, anchor + guard, 1
            ):
                raise VerificationError(
                    "Historical workflow may only add the exact camera transition guard"
                )
