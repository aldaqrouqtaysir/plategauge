...
class ApprovalTests(ReleaseFixture):
    def setUp(self) -> None:
        ...
        for path, (anchor, _) in gate.WORKFLOW_GUARDS.items():
            file = self.repo / path
            file.parent.mkdir(parents=True, exist_ok=True)
            file.write_bytes(b"jobs:\n" + anchor + b"    runs-on: ubuntu-latest\n")
        self.commit("base")
    ...
    def test_exact_historical_workflow_guards_are_allowed(self) -> None:
        for path, (anchor, guard) in gate.WORKFLOW_GUARDS.items():
            file = self.repo / path
            file.write_bytes(file.read_bytes().replace(anchor, anchor + guard))
