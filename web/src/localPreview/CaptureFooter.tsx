...
export default function CaptureFooter({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <footer className="lp-footer">
    <nav aria-label="Privacy and project information">
      <a href={`${base}legal/CAMERA_PRIVACY_NOTICE.md`} onClick={(event) => { if (isCurrentDocumentNavigation(event)) onNavigate?.(); }}>Privacy</a>
      <a href={`${base}legal/NOTICE.txt`} onClick={(event) => { if (isCurrentDocumentNavigation(event)) onNavigate?.(); }}>Attribution & notices</a>
      <a href={source} onClick={(event) => { if (isCurrentDocumentNavigation(event)) onNavigate?.(); }} rel="noreferrer">Source</a>
    </nav>
